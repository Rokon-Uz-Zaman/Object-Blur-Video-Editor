...
def trackingfun():
    ...
    while True:
        ...
        #frame = cv2.putText(frame,str(size), (20, 100), font, 2, (0, 0, 255))
    ...
    #out = cv2.VideoWriter('outputFile3.avi', cv2.VideoWriter_fourcc('M', 'J', 'P', 'G'), 10000, size)
...
label37 = tk.Label(root, text='                   ', width=40)
label37.grid(row=5, column=1)
label37 = tk.Label(root, text='                   ', width=40)
label37.grid(row=6, column=1)
buttonplay = tk.Button(root, text='Play video file ', width=30, command=lambda: trackingfun())
buttonplay.grid(row=7, column=1)
label35 = tk.Label(root, text='                        ', width=40)
label35.grid(row=8, column=1)
